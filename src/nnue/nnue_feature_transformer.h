...
class FeatureTransformer {
   ...
   private:
    ...
    try_find_computed_accumulator(const Position& pos, bool psqtOnly) const {
        ...
        while (st->previous
               && (!st->accumulator.computedPSQT[Perspective]
                   || (!psqtOnly && !st->accumulator.computed[Perspective])))
        {
    ...
    void update_accumulator_incremental(const Position& pos,
...

        {
            ...
            for (; i >= 0; --i)
            {
                states_to_update[i]->accumulator.computed[Perspective]     = !psqtOnly;
                states_to_update[i]->accumulator.computedPSQT[Perspective] = true;
